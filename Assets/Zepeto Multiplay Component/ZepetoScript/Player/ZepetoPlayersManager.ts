...
export default class ZepetoPlayersManager extends ZepetoScriptBehaviour {
    ...
    GetPlayer(number:string):Player
    {
        const player = this._currentPlayers.get(number)
        console.log(`${player.sessionId} - ${player.zepetoUserId}`)
        return player;
    }
    private Awake() {
    ...
    IsReady()
    {
        if(this._currentPlayers.get(this.mySessionId)) 
        {
            return true
        }
        else{
